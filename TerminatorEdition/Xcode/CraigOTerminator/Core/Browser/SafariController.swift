...
/// Controller for Safari browser automation
final class SafariController: BrowserController {
    let app: BrowserApp = .safari
    ...
    func getAllTabs() async throws -> [BrowserTab] {
        ...
        do {
            ...
            return try parseSafariTabs(result)
    ...
    func getHeavyTabs() async throws -> [BrowserTab] {
        // Safari doesn't expose memory usage via AppleScript
    ...
    private func parseSafariTabs(_ result: String) throws -> [BrowserTab] {
        // Safari returns format: URL, title, windowIndex, tabIndex
