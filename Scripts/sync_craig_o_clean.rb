#!/usr/bin/env ruby
# Automated Xcode Project Sync for Craig-O-Clean
# Syncs Swift files from Craig-O-Clean/ to Craig-O-Clean.xcodeproj
...
OptionParser.new do |opts|
  opts.banner = "Usage: sync_craig_o_clean.rb [options]"
...
def find_swift_files(project_dir, exclude_tests: false)
  ...
  Dir.glob("#{project_dir}/**/*.swift").each do |file|
    ...
    if exclude_tests && (file.include?('/Tests/') || file.include?('Test.swift'))
      next
    end
...
def find_or_create_group(project, source_dir, group_parts, dry_run: false)
  ...
  craig_group = group.groups.find { |g| g.name == 'Craig-O-Clean' || g.path == 'Craig-O-Clean' }

  unless craig_group
    log("Error: Could not find Craig-O-Clean group", RED, "❌ ")
    exit 1
  end
...
def main
  ...
  project_path = File.join(project_root, 'Craig-O-Clean.xcodeproj')
  source_dir = File.join(project_root, 'Craig-O-Clean')
  ...
  log("🔄 Craig-O-Clean Xcode Auto-Sync", BLUE)
  ...
  target = project.targets.find { |t| t.name == 'Craig-O-Clean' }

  unless target
    log("Error: Could not find Craig-O-Clean target", RED, "❌ ")
    exit 1
  end
